...
def read_labchart_txt(file_path: str, delimiter: str = '\t') -> pd.DataFrame:
    ...
    It robustly handles rows with more or fewer columns than the header.
    """
    ...
    # --- REFACTORED ROBUST PARSING LOGIC ---
    
    # If no channels found in header, try to guess from the first data line
    ...
    # Define the final column structure based *only* on the header (or our best guess)
    ...
    for line_num, line in enumerate(lines[data_start_index:], start=data_start_index):
        ...
        if row_len == num_expected_cols:
            ...
            # Missing comment, add None
            parts.append(None)
            data_rows.append(parts)
        
        elif row_len > num_expected_cols:
            # Too many columns (This fixes your error)
            # Assume extra columns are part of a malformed comment
            warnings.warn(f"Line {line_num+1}: Row has {row_len} cols, expected {num_expected_cols}. Truncating/merging comment.")
            data_part = parts[:num_expected_cols - 1]
            comment_part = " ".join(parts[num_expected_cols - 1:]) # Join all remaining parts
            data_rows.append(data_part + [comment_part])
        
        elif row_len < num_expected_cols - 1:
            # Too few columns (missing data)
            #warnings.warn(f"Line {line_num+1}: Row has {row_len} cols, expected {num_expected_cols}. Padding with None.")
    ...
    for col in df.columns:
        if col != 'Comment':
            # Use .replace with regex=True for robustness
...
def tms_csp(
    ...
    Parameters:
    # ... (other parameters)
    csp_method (str): Method for CSP threshold detection. Options: 
                      'median' (uses `csp_threshold_percentile`), 
                      'sd' (uses `csp_sd_multiplier`), or
                      'mcd' (uses `csp_mcd_multiplier`).
    # ... (other parameters)
    csp_mcd_multiplier (float): The multiplier for the Mean Consecutive Difference (MCD) method,
                                based on Garvey et al. 2001. Default is 2.66 (3-SD equivalent).
    # ... (other parameters)
    plot_rectified_emg (bool): If True, plots the light grey rectified EMG trace 
                               underneath the raw EMG signal.
    """
    ...
    for file_path in file_paths:
        ...
        for section_num, section_df in enumerate(sections, 1):
            ...
            # Calculate sampling rate and min samples for this section
            ...
            for i, stim in enumerate(all_stimuli):
                ...
                if stim_type == 'M-Wave':
                    ...
                    if not short_pre_stim_df.empty:
                        ...
                        # CSP DURATION LOGIC (SMOOTHING + DURATION)
                        ...
                        if not csp_search_df.empty:
                            ...
                            # 2. Check when smoothed signal is above threshold
                            above_threshold = (smoothed_emg > csp_threshold_value)
                            
                            # 3. Check for *duration* of being above threshold
                            ...
                            if first_window_end_index is not None:
                                ...
                                # Get the integer *position* of this index within csp_search_df
                                end_position = csp_search_df.index.get_loc(first_window_end_index)
                                
                                # The start position is `duration_samples_needed - 1` samples *before* it
                                start_position = end_position - (duration_samples_needed - 1)
                                
                                # Get the original index label at that start position
                                start_index = csp_search_df.index[start_position]
                                
                                # This index label corresponds to the *start* of the return to activity.
                    ...
                    # Plot grey box first so it's in the background
                    if csp_end_time:
