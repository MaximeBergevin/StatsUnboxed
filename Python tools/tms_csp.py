...
def tms_csp(
    ...
    """
    Analyzes LabChart .txt files in a single, chronological pass per section,
    plotting M-waves and MEPs in the same figure and handling all calculations.
    """
    ...
    for file_path in file_paths:
        ...
        try:
            ...
            if local_force_column and local_force_column not in df.columns:
                print(f"  - Warning: Force column '{local_force_column}' not found.")
        ...
        for section_num, section_df in enumerate(sections, 1):
            ...
            # Find and sort all stimuli in the section
            # ----------------------------------------
            all_stimuli = []
            ...
            # Find Mmax for this section
            # ----------------------------
            m_max_for_section = None
            m_waves_in_section = [s for s in all_stimuli if s['type'] == 'M-Wave']
            if m_waves_in_section:
                m_wave_amplitudes = []
                for mw in m_waves_in_section:
                    m_wave_df = section_df[(section_df[time_column_name] >= mw['time']) & (section_df[time_column_name] <= mw['time'] + 0.050)]
                    if not m_wave_df.empty:
                        m_wave_amplitudes.append(m_wave_df[emg_column_name].max() - m_wave_df[emg_column_name].min())
                if m_wave_amplitudes:
                    m_max_for_section = max(m_wave_amplitudes)
                    print(f"  - Section #{section_num}: Found {len(m_wave_amplitudes)} M-wave(s). Mmax for this section is {m_max_for_section:.4f}")
            else:
                print(f"  - Section #{section_num}: No M-waves found in this section. Normalization will be skipped.")

            print(f"  - Section #{section_num}: Found {len(all_stimuli)} total stimuli. Analyzing and plotting...")
            
            # Setup one figure for all stimuli in this section
            plots_per_stim = 2 if local_force_column else 1
            num_rows = len(all_stimuli) * plots_per_stim
            fig_height = (3 * len(all_stimuli)) * plots_per_stim
            fig, axes = plt.subplots(nrows=num_rows, ncols=1, figsize=(12, fig_height), squeeze=False)
            axes = axes.flatten()

            # Chronological Analysis and Plotting Loop
            # ----------------------------------------
            for i, stim in enumerate(all_stimuli):
                pulse_time = stim['time']
                stim_type = stim['type']
                
                ax_idx = i * plots_per_stim
                ax_emg = axes[ax_idx]
                plot_window_df = section_df[(section_df[time_column_name] >= pulse_time - 0.2) & (section_df[time_column_name] <= pulse_time + 0.5)]

                if stim_type == 'M-Wave':
                    # Analyze and plot M-Wave
                    ...
                    # MEP Amplitude
                    mep_df = section_df[(section_df[time_column_name] >= pulse_time + 0.005) & (section_df[time_column_name] <= pulse_time + 0.050)]
                    mep_amplitude = mep_df[emg_column_name].max() - mep_df[emg_column_name].min() if not mep_df.empty else 0
                    ...
                    short_pre_stim_df = section_df[(section_df[time_column_name] >= pulse_time - 0.120) & (section_df[time_column_name] < pulse_time - 0.020)]
                    ...
                    short_pre_stim_emg_rms, short_pre_stim_mean_force = float('nan'), float('nan')
                    long_pre_stim_emg_rms, long_pre_stim_mean_force, long_pre_stim_force_cv = float('nan'), float('nan'), float('nan')
                    ...
                    if not short_pre_stim_df.empty:
                        ...
                        onset_search_df = section_df[(section_df[time_column_name] >= pulse_time + 0.010) & (section_df[time_column_name] <= pulse_time + 0.050)]
                        ...
                        if auc_start_time is None or auc_end_time is None:
                            auc_start_time, auc_end_time = pulse_time + 0.015, pulse_time + 0.100
                    ...
                    if not short_pre_stim_df.empty:
                        ...
                        csp_search_start = pulse_time + (mep_window_ms[1] / 1000.0) + 0.05 # Add 50ms buffer after MEP window
                    ...
                    if m_max_for_section and m_max_for_section > 0:
                        mep_amplitude_normalized = (mep_amplitude / m_max_for_section) * 100
                        mep_auc_normalized = (mep_auc / m_max_for_section) * 100
                    ...
                    results_to_append = {
                        ...
                        'mmax_used': m_max_for_section, 
                        'short_pre_stim_emg_rms': short_pre_stim_emg_rms, 'long_pre_stim_emg_rms': long_pre_stim_emg_rms,
                    }
                    if local_force_column:
