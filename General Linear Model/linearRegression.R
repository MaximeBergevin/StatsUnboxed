...
## as well as the age and the VO2max of 15 participants.

set.seed(42)
n <- 15 # Number of participants
simpleRegression.data <- tibble(
  id = 1:n,
  # Generate a set of 15 random normally-distributed number for both variables
  vo2max = rnorm(n, mean = 35, sd = 10),
  pain = 7 - scale(vo2max)[,1] + rnorm(n, mean = 0, sd = 1)
)
...
vo2max.vector <- simpleRegression.data$vo2max
pain.vector <- simpleRegression.data$pain

# Linear algebra
X <- cbind(1, vo2max.vector)   # Design matrix (X)
...
regression.lm <- lm(data = simpleRegression.data, pain ~ vo2max)
...
ggsave(simpleRegressionResiduals.plot, file = 'General Linear Model/Figures/plot_regression-residuals.tiff',
